...
// 同源部署：默认走 /api/judge（Pages Function），前端零跨域。
// 仍保留 window.__API_BASE 覆盖口子，方便本地指向 wrangler pages dev 或其他代理。
const API_BASE = (typeof window !== 'undefined' && window.__API_BASE) || '';

// 裁决请求的硬上限：接口正常在 2~5s 返回，给移动网络留足余量。
// 超过就放弃 AI、走本地兜底 —— 宁可文案差一点，也不能把人留在转圈里。
const AI_TIMEOUT_MS = 15000;

async function judgeByAI(wish) {
  // 裸 fetch 遇到「连接被挂住、既不返回也不报错」时永远不 settle（大陆访问
  // workers.dev 常见），上层的 await 就会一直悬着、加载态永不解除。
  // 用 AbortController 给它一个硬超时，超时后主动中断并抛错，
  // 交给 judgeWish 的 catch 落到 fallbackJudge。
  ...
  try {
    const res = await fetch(`${API_BASE}/api/judge`, {
      ...
      body: JSON.stringify({ wish, lang: LANG }),
      signal: ctrl.signal
    });
...
const EASTER_EGGS = [
  ...
  { re: /乔乔头少/, zh: '天芥爱你，真心可鉴', en: 'Tianjie loves you, and the sincerity shows' },
  { re: /苏蕊/, zh: '一个俄文ID的人在等着你', en: 'Someone with a Russian ID is waiting for you' },
...
$('btnBreak').addEventListener('click', async () => {
  ...
  // 加载态放进 try/finally：无论裁决成功、失败还是抛出意外异常，
  // 转圈都必须解除，从结构上杜绝「永久卡在审视中」
  let verdict;
  try {
    verdict = await judgeWish(wishStr);
  } catch (e) {
    // judgeWish 内部已有兜底，正常到不了这里；这层是防御未来改动引入新异常
    console.warn('裁决流程异常，使用本地后备:', e);
    verdict = fallbackJudge(wishStr);
  } finally {
    judging.classList.remove('visible');
